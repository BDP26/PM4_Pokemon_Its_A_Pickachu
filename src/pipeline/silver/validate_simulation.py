...
def validate_simulation_artifacts(silver_dir: Path = SILVER_DIR) -> list[str]:
    ...
    for idx, team in enumerate(teams, start=1):
        ...
        if not _is_sequence_like(pokemon) or len(pokemon) == 0:
    ...
    for idx, seed in enumerate(seeds, start=1):
        ...
        if not _is_numeric(win_chance) or not (0.0 <= float(win_chance) <= 1.0):
    ...
    for idx, mc_row in enumerate(monte_carlo, start=1):
        ...
        if not _is_numeric(base_prob) or not (0.0 <= float(base_prob) <= 1.0):
            issues.append(f"monte_carlo_results.parquet row {idx}: predicted_player_win_chance out of [0, 1]")

        if not _is_numeric(n_trials) or int(n_trials) <= 0:
        ...
        if not _is_numeric(mc_win_rate) or not (0.0 <= float(mc_win_rate) <= 1.0):
