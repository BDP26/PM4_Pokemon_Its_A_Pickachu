...
from src.pipeline.gold.build_walkthrough_web import build_walkthrough_best_teams_payload
...
def build_gold_from_silver(silver_dir: Path = SILVER_DIR, gold_dir: Path = GOLD_DIR) -> None:
    ...
    logger.info("[gold] wrote game_progression_summary.csv rows=%s", len(progression))
    ...
    logger.info("[gold] wrote location_popularity.parquet rows=%s", len(location_popularity))
    ...
    if monte_carlo_path.exists():
        ...
        if not monte_carlo_df.empty:
            ...
            logger.warning("[gold] monte_carlo_results.jsonl is empty; skipping recommendation outputs")
    else:
    ...
    logger.info("[gold] wrote %s datasets from %s silver files", len(manifest["gold_outputs"]), len(game_files))
